...
function Map() {
	...
	async function getRestuarantMainImg (photo_reference) {
		...
		if (photo_reference !== undefined) {
			console.log('img:', await axios.get("http://localhost:3100/api/v1/restaurant/photo/" + photo_reference).data)
			img = await axios.get("http://localhost:3100/api/v1/restaurant/photo/" + photo_reference).data
			return img
			// console.log(img)
		...
		<div style={ {'display': 'flex', 'flex-direction': 'row'} }>
			...
			<div>
				...
				<svg height="200vh" width="65vw" font-weight='600'>
					...
					<g id='Ximen' font-size="15" onClick={() => handleClickOnStation({'english': 'Ximen', 'chinese': '西門'})}>
